...
# ── Structured PO dataset ─────────────────────────────────────────────────────
# po_data.json is generated by build_po_data.py from the source PO documents in
# POs/. It is the source of truth for per-type and per-branch Programme Outcomes.
# The hardcoded POS_BY_TYPE above remains a fallback for any type absent from it.
_PO_DATA_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "po_data.json")


def _load_po_data() -> dict:
    try:
        with open(_PO_DATA_PATH, encoding="utf-8") as f:
            return json.load(f)
    except (OSError, ValueError):
        return {}


PO_DATA = _load_po_data()


def _po_tuples(rows) -> list:
    """Convert JSON [key, name, statement] rows into (key, name, desc) tuples."""
    return [tuple(r) for r in rows]


# Branches available under the combined "Science & Arts (Degree)" type, derived
# from the dataset. Each entry: (code, label, po_stream).
SCIENCE_ARTS_BRANCHES = [
    (code, b.get("label", code), b.get("stream", "science"))
    for code, b in PO_DATA.get("science_arts", {}).get("branches", {}).items()
]
# code -> (label, po_stream) lookup
BRANCH_BY_CODE = {code: (label, stream) for code, label, stream in SCIENCE_ARTS_BRANCHES}


def get_branch_label(branch_code: str) -> str:
    """Return the human-readable label for a branch code (or '' if unknown)."""
    entry = BRANCH_BY_CODE.get(branch_code or "")
    return entry[0] if entry else ""


def get_pos_for_type(college_type: str, branch: str = None) -> list:
    """Return the PO list for a given college type (defaults to engineering).

    Prefers the structured dataset (po_data.json, parsed from each college's PO
    document). For the combined "science_arts" type the PO set comes from the
    selected branch. Falls back to the curated hardcoded POS_BY_TYPE for any
    type/branch not present in the dataset.
    """
    ct = college_type or "engineering"
    section = PO_DATA.get(ct)
    if section:
        if section.get("mode") == "branched":
            entry = section.get("branches", {}).get(branch or "")
            if entry:
                return _po_tuples(entry["pos"])
        elif section.get("pos"):
            return _po_tuples(section["pos"])
    # Fallbacks when the dataset lacks this type/branch.
    if ct == "science_arts":
        entry = BRANCH_BY_CODE.get(branch or "")
        stream = entry[1] if entry else "science"
        return POS_BY_TYPE.get(stream, POS)
    return POS_BY_TYPE.get(ct, POS)
